...
const fetchData = async () => {
    try {
        ...
        query.append('action', 'mobile-notification-all');
        if (role) query.append('role', role);
        if (branchId) query.append('branch_id', branchId);
        const {status, data} = await axiosConfig.get(`?${query.toString()}`).catch(error => {
            console.error('❌ UseNotification axios Error:', error.response?.data || error.message);
            throw error;
        });
        if (status !== 200) {
        ...
        console.error('❌ UseNotification fetchData Error:', error);
        throw error;
    }
};
